 ...
 * @LastEditTime: 2022-01-03 18:04:38
-->
<template>
  <div ref="eleTable">
          ...
          <template v-for="(item, index) in filterColumns" :key="index">
            ...
            >
              <template #default="scope">
                ...
                <template v-else>
                  <Ex-Pand :column="item" :row="scope.row" :render="item.render" :index="scope.$index" />
                </template>
              </template>
            </el-table-column>
          </template>
...
/**表单规则与数据 (START) */
    state.rule = computed(() => {
      ...
      return [...props.config.form.rule]}).value
    state.option = computed(() => {
      if(!props.config.form) return 
      return props.config.form.option}).value
/**表单规则与数据 (END) */
...
/**每页展现条数选择项 */
    ...
    const getData = async (params) => {
      ...
      if(props.config.form) {
        data = await store.dispatch(props.action, {pageNum: state.pageNum,pageSize: state.mPageSize, ...proxy.formObj.formData()})
        console.log('data', data)
      } else {
        data = await store.dispatch(props.action, {pageNum: state.pageNum, pageSize: state.mPageSize, ...params})
        console.log('data', data)
      }
      // const { data, err} = await store.dispatch(props.action, params)
     state.loading = false
      if (data.err) return ElMessage({showClose: true,message: err,type: 'error',center: true,duration: 3000})
